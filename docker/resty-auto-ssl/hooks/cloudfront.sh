...
deploy_cert() {
    ...
    cp "${KEYFILE}" "${FULLCHAINFILE}" /etc/ssl/
    nginx -s reload
}
...
invalid_challenge() {
    ...
    # Simple example: Print error message
    printf "SSL CERT: Validation of ${DOMAIN} failed!"
}
...
request_failure() {
    ...
    # Simple example: Print error
    printf "SSL CERT: HTTP request failed failed with status ${STATUSCODE}!"
}
